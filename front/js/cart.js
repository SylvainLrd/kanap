...
async function printCart() {
  const getcart = document.getElementById("cart__items");
  getcart.innerHTML = ``;
  let totalPrice = 0;
  let totalQty = 0;
  const productLocalStorage = JSON.parse(localStorage.getItem("product"));
  if (!productLocalStorage) {
    ...
    try {
      ...
      for (product of productLocalStorage) {
        const rechercheProduit = data.find((item) => item._id === product.id);
        getcart.innerHTML += `
        ...
        totalPrice += rechercheProduit.price * product.qty;
